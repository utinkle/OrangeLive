...
typedef int (*av_jni_set_java_vm_func)(JavaVM*, void*);

void setupFFmpegJNI(JavaVM* vm)
{
    QLibrary library("libavcodec");

    if (!library.load()) {
        qCritical() << "Failed to load library" << library.errorString();
        return;
    }

    auto av_jni_set_java_vm_ptr = (av_jni_set_java_vm_func)library.resolve("av_jni_set_java_vm");
    if (!av_jni_set_java_vm_ptr) {
        qCritical() << "Failed to resolve symbol av_jni_set_java_vm";
        return;
    }

    av_jni_set_java_vm_ptr(vm, nullptr);
    qDebug() << "Successfully registered JavaVM with FFmpeg.";
}
#endif
...
void MpvController::init()
{
    ...
    mpv_set_option_string(d_ptr->m_mpv, "hwdec", "mediacodec-copy");
    ...
    configPath.append(QStringLiteral("/olmpv"));
    configPath.append(QStringLiteral("/olmpv.conf"));
    setProperty(QStringLiteral("include"), configPath);
    setProperty(QStringLiteral("vo"), QStringLiteral("libmpv"));
#ifdef Q_OS_ANDROID
    setProperty(QStringLiteral("ao"), QStringLiteral("opensles"));
#endif
    d_ptr->m_mpvHandleManager = std::make_shared<MpvHandleManager>(d_ptr->m_mpv);

    mpv_request_log_messages(d_ptr->m_mpv, "debug");
    mpv_set_wakeup_callback(d_ptr->m_mpv, MpvController::mpvEvents, this);
}
...
void MpvController::eventHandler()
{
    while (d_ptr->m_mpv) {
        ...
        case MPV_EVENT_LOG_MESSAGE: {
            ...
            QString text = QString::fromUtf8(msg->text).trimmed(); // 去除末尾换行符

            qDebug().noquote() << QStringLiteral("[MPV %1][%2] %3").arg(level, prefix, text);
            break;
        }
